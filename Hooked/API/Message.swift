...
class Message {
    ...
    init(id: String, from: String, date: Double, text: String, imageUrl: String, height: Double, width: Double, videoUrl: String) {
        self.id = id
        self.from = from
        //self.to = to
        self.date = date
    ...
    static func transformMessage(dict: [String: Any], keyId: String) -> Message? {
        ...
        let message = Message(id: keyId, from: from, date: date, text: text, imageUrl: imageUrl, height: height, width: width, videoUrl: videoUrl)
        return message
    }
}
